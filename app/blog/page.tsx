...
export default function Blog() {
  ...
  useEffect(() => {
    DataService.getPosts().then(setPosts);
  }, []);
  ...
  const filteredPosts = posts.filter(post => {
    ...
    return matchesFilter && matchesSearch && post.status === 'published';
  });
          ...
          filteredPosts.map((post) => (
            <article key={post.id} className="group grid md:grid-cols-[1fr_2fr] gap-6 md:gap-8 items-start">
              <Link href={`/blog/${post.slug}`} className="block aspect-[4/3] rounded-xl overflow-hidden bg-zinc-800">
                <Image src={post.imageUrl || 'https://via.placeholder.com/800x600'} alt={post.title} width={800} height={800} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 opacity-80 group-hover:opacity-100" />
              </Link>
